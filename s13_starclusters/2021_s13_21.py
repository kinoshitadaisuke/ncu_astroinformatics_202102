...
#
# Time-stamp: <2021/05/30 23:54:30 (CST) daisuke>
#
...
for i in range ( len (data_distance) ):
    ...
    if ( (data_distance[i] >= x_min) and (data_distance[i] < x_max) ):
        print ("%s %f %f %f %f %f %f %f %f %f %f %f %f" \
               % (data_id[i], data_ra[i], data_dec[i], data_parallax[i], \
                  data_pmra[i], data_pmdec[i], data_rv[i], \
                  data_b[i], data_g[i], data_r[i], \
                  data_br[i], data_bg[i], data_gr[i]) )
